...
const InterviewPage: React.FC = () => {
  ...
  useEffect(() => {
    ...
    const fetchQuestions = async () => {
      try {
        const response = await fetch('https://mock-interview-backend-4.onrender.com/api/questions');
        if (!response.ok) {
  ...
  const handleFinishInterview = async () => {
    try {
        const response = await fetch('https://mock-interview-backend-4.onrender.com/api/analyze-transcript', {
            method: 'POST',
            headers: {
  ...
  const handleStopRecording = async () => {
    ...
    if (mediaRecorderRef.current) {
      ...
      try {
        const response = await fetch('https://mock-interview-backend-4.onrender.com/api/analyze-answer', {
          method: 'POST',
          headers: {
            'Content-Type': 'application/json',
          },
